...
- (void)generateBlockchainIdentityExtendedPublicKeys:(void (^_Nullable)(BOOL registered))completion {
    ...
    [[DSAuthenticationManager sharedInstance] seedWithPrompt:@"Generate Blockchain Identity"
...
- (void)encryptData:(NSData *)data withKeyAtIndex:(uint32_t)index forRecipientKey:(DSKey *)recipientPublicKey withPrompt:(NSString *_Nullable)prompt completion:(void (^_Nullable)(NSData *encryptedData))completion {
    [[DSAuthenticationManager sharedInstance] seedWithPrompt:@""
                                                   forWallet:self.wallet
                                                   forAmount:0
                                         forceAuthentication:NO
                                                  completion:^(NSData *_Nullable seed, BOOL cancelled) {
...
- (void)fetchAndUpdateContract:(DPContract *)contract {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{ //this is so we don't get DAPINetworkService immediately
        if (contract.contractState == DPContractState_Unknown) {
            [self.DAPINetworkService getIdentityByName:@"dashpay7"
                inDomain:@""
            ...
            [self.DAPINetworkService fetchContractForId:contract.base58ContractID
...
- (void)fetchAndUpdateContractWithIdentifier:(NSString *)identifier {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{ //this is so we don't get DAPINetworkService immediately
        [self.DAPINetworkService fetchContractForId:identifier
...
- (void)registerUsernamesAtStage:(DSBlockchainIdentityUsernameStatus)blockchainIdentityUsernameStatus {
    ...
    switch (blockchainIdentityUsernameStatus) {
        case DSBlockchainIdentityUsernameStatus_Initial: {
            ...
            if (usernames.count) {
                [self registerPreorderedSaltedDomainHashesForUsernames:usernames
                                                            completion:^(BOOL success) {
                                                                if (success) {
        ...
        case DSBlockchainIdentityUsernameStatus_Preordered: {
            ...
            if (usernames.count) {
                [self registerUsernameDomainsForUsernames:usernames
                                               completion:^(BOOL success) {
                                                   if (success) {
        ...
        case DSBlockchainIdentityUsernameStatus_RegistrationPending: {
            ...
            if (usernames.count) {
                [self monitorForDPNSUsernames:usernames
                               withRetryCount:2
                                   completion:^(BOOL success) {
                                       if (success) {
                                           //Done;
...
- (void)fetchUsernamesWithCompletion:(void (^)(BOOL))completion {
    __weak typeof(self) weakSelf = self;
    [self.DAPINetworkService getDPNSDocumentsForIdentityWithUserId:self.uniqueIdString
...
- (void)updateCreditBalance {
    __weak typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{ //this is so we don't get DAPINetworkService immediately
        [self.DAPINetworkService getIdentityById:self.uniqueIdString
...
- (void)monitorForBlockchainIdentityWithRetryCount:(uint32_t)retryCount completion:(void (^)(BOOL))completion {
    __weak typeof(self) weakSelf = self;
    [self.DAPINetworkService getIdentityById:self.uniqueIdString
        success:^(NSDictionary *_Nonnull profileDictionary) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) {
                return;
            }
...
- (void)sendNewFriendRequestMatchingPotentialFriendship:(DSPotentialOneWayFriendship *)potentialFriendship completion:(void (^)(BOOL))completion {
    ...
    [self.DAPIClient sendDocument:potentialFriendship.contactRequestDocument
                      forIdentity:self
                         contract:contract
                       completion:^(NSError *_Nullable error) {
                           ...
                           if (success) {
                               ...
                               [self fetchProfileForBlockchainIdentityUniqueId:potentialFriendship.destinationContact.associatedBlockchainIdentityUniqueId
                                                           saveReturnedProfile:NO
                                                                       context:self.managedObjectContext
                                                                    completion:^(DSContactEntity *contactEntity) {
                                                                        if (!contactEntity) {
                                                                            if (completion) {
                                                                                completion(NO);
                                                                            }
...
- (void)acceptFriendRequest:(DSFriendRequestEntity *)friendRequest completion:(void (^)(BOOL))completion {
    DSAccount *account = [self.wallet accountWithNumber:0];
    DSPotentialContact *contact = [[DSPotentialContact alloc] initWithUsername:friendRequest.sourceContact.username
                                                                    avatarPath:friendRequest.sourceContact.avatarPath
...
- (void)fetchProfile:(void (^)(BOOL))completion {
    __weak typeof(self) weakSelf = self;
    [self fetchProfileForBlockchainIdentityUniqueId:self.uniqueID
                                saveReturnedProfile:TRUE
                                            context:self.managedObjectContext
                                         completion:^(DSContactEntity *contactEntity) {
                                             if (completion) {
                                                 if (contactEntity) {
...
- (void)fetchProfileForBlockchainIdentityUniqueId:(UInt256)blockchainIdentityUniqueId saveReturnedProfile:(BOOL)saveReturnedProfile context:(NSManagedObjectContext *)context completion:(void (^)(DSContactEntity *contactEntity))completion {
    __weak typeof(self) weakSelf = self;
    [self.DAPINetworkService getDashpayProfileForUserId:uint256_reverse_base58(blockchainIdentityUniqueId)
        success:^(NSArray<NSDictionary *> *_Nonnull documents) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) {
                return;
...
- (void)fetchIncomingContactRequests:(void (^)(BOOL success))completion {
    __weak typeof(self) weakSelf = self;
    [self.DAPINetworkService getDashpayIncomingContactRequestsForUserId:self.uniqueIdString
        since:0
        success:^(NSArray<NSDictionary *> *_Nonnull documents) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) {
            ...
            [strongSelf handleContactRequestObjects:documents
                                            context:strongSelf.managedObjectContext
                                         completion:^(BOOL success) {
                                             if (completion) {
                                                 completion(YES);
                                             }
    ...
    [self.managedObjectContext performBlockAndWait:^{
        ...
        for (DSContactRequest *contactRequest in incomingRequests) {
            ...
            if (!externalContact) {
                ...
                dispatch_group_enter(dispatchGroup);
                [self.DAPINetworkService getIdentityById:uint256_base58(contactRequest.senderBlockchainIdentityUniqueId)
                    success:^(NSDictionary *_Nonnull blockchainIdentityDictionary) {
                        ...
                        [self.DAPINetworkService getDPNSDocumentsForIdentityWithUserId:uint256_base58(contactRequest.senderBlockchainIdentityUniqueId)
                            success:^(NSArray<NSDictionary *> *_Nonnull documents) {
                                if ([documents count]) {
                                    NSDictionary *nameDictionary = [documents firstObject];
                                    [self fetchProfileForBlockchainIdentityUniqueId:contactRequest.senderBlockchainIdentityUniqueId
                                                                saveReturnedProfile:NO
                                                                            context:context
                                                                         completion:^(DSContactEntity *contactEntity) {
                                                                             if (contactEntity) {
    ...
    [context performBlockAndWait:^{
        ...
        for (DSContactRequest *contactRequest in outgoingRequests) {
            ...
            if (!destinationContact) {
                //no contact exists yet
                dispatch_group_enter(dispatchGroup);
                [self.DAPINetworkService getIdentityById:uint256_base58(contactRequest.recipientBlockchainIdentityUniqueId)
                    success:^(NSDictionary *_Nonnull blockchainIdentityDictionary) {
                        ...
                        if (blockchainIdentityDictionary) {
                            ...
                            [self fetchProfileForBlockchainIdentityUniqueId:contactBlockchainIdentityUniqueId
                                                        saveReturnedProfile:NO
                                                                    context:context
                                                                 completion:^(DSContactEntity *destinationContactEntity) {
