...
//@property (nonatomic, assign) LLMQTyp llmqType;
...
//}
//
//- (instancetype)initWithVersion:(uint16_t)version type:(DLLMQType)type quorumHash:(UInt256)quorumHash quorumIndex:(uint32_t)quorumIndex quorumPublicKey:(UInt384)quorumPublicKey quorumEntryHash:(UInt256)quorumEntryHash verified:(BOOL)verified onChain:(DSChain *)chain {
//    if (!(self = [super init])) return nil;
//
//    self.llmqType = type;
//    self.version = version;
...
//}
//
////- (instancetype)initWithEntry:(DLLMQEntry *)entry onChain:(DSChain *)chain {
////    if (!(self = [super init])) return nil;
