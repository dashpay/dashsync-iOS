...
BOOL has_contact_request_with_id_caller(const void *context, BOOL incoming, u256 *contact_request_id, const void *storage_context) {
    DSIdentity *identity = AS_OBJC(context);
    NSManagedObjectContext *ctx = ((__bridge NSManagedObjectContext *)(storage_context));
    NSData *identifier = NSDataFromPtr(contact_request_id);
    u256_dtor(contact_request_id);
    __block BOOL exist = NO;
    [ctx performBlockAndWait:^{
        exist = [DSFriendRequestEntity countObjectsInContext:ctx
                                                    matching: incoming
                 ? @"destinationContact == %@ && sourceContact.associatedBlockchainIdentity.uniqueID == %@"
                 : @"sourceContact == %@ && destinationContact.associatedBlockchainIdentity.uniqueID == %@", [identity matchingDashpayUserInContext:ctx], identifier];
    }];
    return exist;
}
void has_contact_request_with_id_dtor(BOOL result) {
    
}
Fn_ARGS_std_os_raw_c_void_bool_Arr_u8_32_std_os_raw_c_void_RTRN_bool has_contact_request_with_id = {
    .caller = &has_contact_request_with_id_caller,
    .destructor = &has_contact_request_with_id_dtor,
};
    ...
    dispatch_group_notify(dispatchGroup, completionQueue, ^{
        ...
        if (!hasMore)
            [self.platformContext performBlockAndWait:^{
                DIdentityModelSetLastCheckedIncomingContactsTimestamp(self.model, [[NSDate date] timeIntervalSince1970]);
            }];
        [debugInfo appendFormat:@" : OK: %u: %@", succeeded, rErrors];
        DSLog(@"%@", debugInfo);
        __block NSData * hasMoreStartAfter = nil;
        if (requests->count > 0) {
            DContactRequest *last = requests->values[requests->count-1];
            if (last)
                hasMoreStartAfter = NSDataFromPtr(last->id);
        }
        DMaybeContactRequestsDtor(result);
...
- (void)fetchOutgoingContactRequestsInContext:(NSManagedObjectContext *)context
    ...
    if (requests->count > 0) {
        dispatch_group_enter(dispatchGroup);
        [self handleOutgoingRequests:requests
                             context:context
                          completion:^(BOOL success, NSArray<NSError *> *errors) {
            if (!success) {
                succeeded = NO;
    ...
    dispatch_group_notify(dispatchGroup, completionQueue, ^{
        ...
        if (!hasMore)
            [self.platformContext performBlockAndWait:^{
                DIdentityModelSetLastCheckedOutgoingContactsTimestamp(self.model, [[NSDate date] timeIntervalSince1970]);
            }];
...
- (void)handleIncomingRequests:(Vec_dash_spv_platform_models_contact_request_ContactRequest *)incomingRequests
    ...
    [context performBlockAndWait:^{
        ...
        dispatch_group_t dispatchGroup = dispatch_group_create();
        for (int i = 0; i < incomingRequests->count; i++) {
            DContactRequest *request = incomingRequests->values[i];
            ...
            if (!externalIdentityEntity) {
                ...
                if ([self.chain identityForUniqueId:externalIdentityEntity.uniqueID.UInt256]) {
                    ...
                    if ([sourceIdentity activeKeyCount] > 0 && [sourceIdentity hasKeyAtIndex:request->sender_key_index]) {
                        ...
                        DMaybeOpaqueKey *extendedPublicKey = [DSKeyManager keyWithExtendedPublicKeyData:decryptedExtendedPublicKeyData ofType:DOpaqueKeyKind(key)];
                        if (!extendedPublicKey) {
                            succeeded = FALSE;
...
- (void)handleOutgoingRequests:(Vec_dash_spv_platform_models_contact_request_ContactRequest *)outgoingRequests
                       context:(NSManagedObjectContext *)context
                    completion:(void (^)(BOOL success, NSArray<NSError *> *errors))completion
             onCompletionQueue:(dispatch_queue_t)completionQueue {
    if (!self.isActive) {
    ...
    [context performBlockAndWait:^{
        ...
        for (int i = 0; i < outgoingRequests->count; i++) {
            DContactRequest *request = outgoingRequests->values[i];
