...
- (instancetype)initWithChainManager:(DSChainManager *)chainManager blockchainUser:(DSBlockchainUser *)blockchainUser {
    ...
    if (self) {
        ...
        // Set contractId and userId on DashPlatformProtocol instance
        DPContract *contract = [DSRegisterDashPayContractModel setDashPayNativeDemo1ContractIfNeeded];
        _contractId = contract.identifier;
        
        DashPlatformProtocol *dpp = [DashPlatformProtocol sharedInstance];
        NSString *regTxHash = [[[NSData dataWithUInt256:blockchainUser.registrationTransactionHash] reverse] hexString];
...
//            @"extendedPublicKey" : ?,
        ...
        __weak typeof(self) weakSelf = self;
        [strongSelf sendDocument:contact contractId:self.contractId completion:^(NSError * _Nullable error) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (!strongSelf) {
                return;
            }
...
- (void)fetchContacts:(void (^)(BOOL success))completion {
    ...
    __weak typeof(self) weakSelf = self;
    [self.chainManager.DAPIClient fetchDocumentsForContractId:self.contractId objectsType:@"contact" options:options success:^(NSArray<NSDictionary *> *_Nonnull documents) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
...
- (void)createProfileWithCompletion:(void (^)(BOOL success))completion {
    ...
    __weak typeof(self) weakSelf = self;
    [self sendDocument:user contractId:self.contractId completion:^(NSError * _Nullable error) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
