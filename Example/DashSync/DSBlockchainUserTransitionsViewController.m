...
@interface DSBlockchainUserTransitionsViewController ()
...
@implementation DSBlockchainUserTransitionsViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.transitions = [self.blockchainUser allTransitions];
...
-(void)refreshStateTransitions {
    [self.chainManager.DAPIClient getAllStateTransitionsForUser:self.blockchainUser completion:^(NSError * _Nullable error) {
        if (!error) {
            self.transitions = [self.blockchainUser allTransitions];
