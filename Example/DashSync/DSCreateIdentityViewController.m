//
//  DSCreateIdentityViewController.m
//  DashSync_Example
//
...
@implementation DSCreateIdentityViewController
...
- (void)viewDidLoad {
    ...
    self.indexLabel.text = [NSString stringWithFormat:@"%d", [self.wallet unusedIdentityIndex]];
...
- (IBAction)done:(id)sender {
    ...
    DSIdentity *identity = [self.wallet createIdentityForUsername:desiredUsername usingDerivationIndex:[self.indexLabel.text intValue]];
    DSIdentityRegistrationStep steps = DSIdentityRegistrationStep_L1Steps;
    if (self.shouldRegisterOnL2) {
        steps |= DSIdentityRegistrationStep_Identity;
    }
