...
- (void)testSimpleReorg {
    ...
    for (DSTransaction * transaction in blockFork.transactions) {
        [self.chain.chainManager.transactionManager peer:peer relayedTransaction:transaction inBlock:blockFork transactionIsRequestingInstantSendLock:YES];
    }
...
- (void)testComplexReorg {
    ...
    for (DSTransaction * transaction in blockFork106.transactions) {
        [self.chain.chainManager.transactionManager peer:peer relayedTransaction:transaction inBlock:blockFork106 transactionIsRequestingInstantSendLock:YES];
    }
    XCTAssertEqual(self.wallet.balance, 10000000000); //Only 1 transaction, coinbase is still locked
    ...
    for (DSTransaction * transaction in blockFork110.transactions) {
        [self.chain.chainManager.transactionManager peer:peer relayedTransaction:transaction inBlock:blockFork110 transactionIsRequestingInstantSendLock:YES];
    }
    ...
    for (DSTransaction * transaction in blockFork106Extra.transactions) {
        [self.chain.chainManager.transactionManager peer:peer relayedTransaction:transaction inBlock:blockFork106Extra transactionIsRequestingInstantSendLock:YES];
    }
